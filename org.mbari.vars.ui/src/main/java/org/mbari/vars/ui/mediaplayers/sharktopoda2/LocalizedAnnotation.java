...
public record LocalizedAnnotation(Annotation annotation, Association association) {
    ...
    public static List<LocalizedAnnotation> from(Annotation a) {
        ...
        for (Association ass: a.getAssociations()) {
            if (ass.getLinkName().equalsIgnoreCase(BoundingBox.LINK_NAME) &&
                ...
                var x = new org.mbari.vars.ui.mediaplayers.sharktopoda.localization.LocalizedAnnotation(a, ass);
                xs.add(x);
            }
        }
        return xs;
    ...
    public static Tuple2<Integer, Optional<LocalizedAnnotation>> search(List<org.mbari.vars.ui.mediaplayers.sharktopoda.localization.LocalizedAnnotation> xs, UUID associationUuid) {
